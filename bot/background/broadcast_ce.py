...
from bot.utils.ce_preview import get_preview
from bot import LOGGER, BOT_NAME_TAG_VER, db_path, ce_board_link
...
async def broadcast_ce(bot):
    """ CE게시판 새 글 알림 전송 """
    if not os.path.exists(db_path):
        await asyncio.sleep(5)
    while True:
        latest_data_id = ceBoardDB().get_latest_data_id()
        # None 이 아닐 경우 반복문 탈출
    ...
    while True:
        while True:
            now_latest_data_id = ceBoardDB().get_latest_data_id()
            # None 이 아닐 경우 반복문 탈출
        ...
        if latest_data_id != now_latest_data_id:
            for data_id in range(latest_data_id + 1, now_latest_data_id + 1):
                # get post
                post = ceBoardDB().get_database_from_id(data_id)
                # 데이터베이스에 정보가 존재할 경우
                if post is not None:
                    try:
